...
namespace SharedMatrix
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            NaiveMatrix matrixA = new(widthA, heightA);
            NaiveMatrix matrixB = new(widthB, heightB);

            NaiveMatrix res_net = new(widthB, heightA);
            NaiveMatrix res_cuda = new(widthB, heightA);

            double numberCompute = matrixA.Height * (double)matrixA.Width * matrixB.Width * 3.0E-9;
        ...
        [EntryPoint]
        public static void Multiply([In, Out] NaiveMatrix result, [In] NaiveMatrix A, [In] NaiveMatrix B, int size)
        {
